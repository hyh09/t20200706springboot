...
public class ExcelRowPoiImpl implements ExcelRow {
	...
	private final ExcelSheet toSheet;
    /**
	...
	public ExcelRowPoiImpl(Row row, ExcelSheet toSheet){
		toRow = row;
		this.toSheet = toSheet;
		if(null == row || null == toSheet) {
	...
	public ExcelCell createCell(int column, int type){
		...
		Cell cell = toRow.createCell(column, type);
	...
	public ExcelCell createCell(int column){
		ExcelCell existCell = cells.get(column);
		if( existCell != null ) return existCell;
		Cell cell = toRow.createCell(column);
		if( null == cell ) return null;
	...
	public ExcelCell getCell(int cellnum) {
		if(cellnum < 0 || cellnum >= toRow.getLastCellNum()) return null;
	...
	public void removeCell(ExcelCell cell){
		toRow.removeCell(((ExcelCellPoiImpl)cell).getPoiCell());
